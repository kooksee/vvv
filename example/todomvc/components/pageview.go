...
func (p *PageView) onPrint(s string) {
	fmt.Println(p.NewItemTitle)
	fmt.Println(s)
}
...
func (p *PageView) Render() vecty.ComponentOrHTML {
	return elem.Body(
		elem.Section(
			vecty.Markup(vecty.Class("todoapp")),
		...
		elem.Form(
			...
			elem.Input(
				vecty.Markup(
					...
					//view.BindValue(p, "NewItemTitle", p.onPrint),
					view.BindValue(&p.NewItemTitle),
					//BindInput(p, &p.newItemTitle),
